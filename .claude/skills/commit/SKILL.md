...
don't run tests/build/lint first, and don't ask for confirmation before pushing: invoking
this skill *is* the confirmation.
...
3. Write a short, imperative commit message matching this repo's style (see `git log`,
   e.g. "Add light entity component; retire the global light slider"): one line, body
   only if the diff does something non-obvious.
4. Commit via heredoc with the footer:
   `Co-Authored-By: Claude Sonnet 5 <noreply@anthropic.com>`
5. `git push`.
6. Report the commit hash and branch in one line. Nothing else unless push fails.
