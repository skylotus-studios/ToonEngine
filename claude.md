...
This file, `MEMORY.md`, `ARCHIVE.md`, `.claude/`, and both style guides are gitignored on
`develop` and live in the `backup` worktree at `../ToonEngine-backup`, symlinked into place.
They are kept on every branch and pushed on none. Editing any of them changes the `backup`
branch, not `develop`.
