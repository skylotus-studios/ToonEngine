...
// Key name <-> enum, populated once on first use.
struct KeyNameTable {
...
Binding BindingFromJson(const json& j) {
    ...
    if (type == "key") {
        const std::string name = j.value("key", "");
        const auto& tbl = Table();
        const auto it = tbl.byName.find(name);
        return KeyBinding{it != tbl.byName.end() ? it->second : Key::Unknown};
    }
    if (type == "mouse_button") {
        const std::string name = j.value("button", "");
    ...
    if (type == "gamepad_button") {
        const int pad = j.value("pad", 0);
        const std::string name = j.value("button", "");
        GamepadButton btn = GamepadButton::A;
        for (int i = 0; i < static_cast<int>(GamepadButton::Count); ++i) {
            if (GamepadButtonName(static_cast<GamepadButton>(i)) == name) { btn = static_cast<GamepadButton>(i); break; }
        }
        return GamepadButtonBinding{pad, btn};
    }
    if (type == "gamepad_axis") {
        const int pad = j.value("pad", 0);
        const std::string name = j.value("axis", "");
        GamepadAxis ax = GamepadAxis::LeftX;
        for (int i = 0; i < static_cast<int>(GamepadAxis::Count); ++i) {
            if (GamepadAxisName(static_cast<GamepadAxis>(i)) == name) { ax = static_cast<GamepadAxis>(i); break; }
        }
        return GamepadAxisBinding{pad, ax, j.value("negative", false), j.value("deadzone", 0.15f)};
    }
    if (type == "mouse_axis") {
        const std::string name = j.value("axis", "");
        MouseAxis ax = MouseAxis::X;
        for (int i = 0; i < static_cast<int>(MouseAxis::Count); ++i) {
            if (MouseAxisName(static_cast<MouseAxis>(i)) == name) { ax = static_cast<MouseAxis>(i); break; }
        }
...
const char* KeyName(Key k) {
    const auto& tbl = Table();
    const auto it = tbl.byCode.find(static_cast<int>(k));
...
bool Save(const char* path, const InputContext& ctx) {
    ...
    for (const auto& [id, action] : ctx.map.actions) {
        json arr = json::array();
        for (const Binding& b : action.bindings) { arr.push_back(BindingToJson(b)); }
        actionsJ[action.name] = arr;
    }
    root["actions"] = actionsJ;

    json axesJ = json::object();
    for (const auto& [id, axis] : ctx.map.axes) {
        json pos = json::array(), neg = json::array();
        for (const Binding& b : axis.positive) { pos.push_back(BindingToJson(b)); }
        for (const Binding& b : axis.negative) { neg.push_back(BindingToJson(b)); }
...
bool Load(const char* path, InputContext& ctx) {
    ...
    try {
        ...
        loaded.name = root.value("context", ctx.name);

        if (root.contains("actions")) {
            for (const auto& [name, arr] : root.at("actions").items()) {
                for (const auto& bj : arr) { loaded.map.BindAction(name.c_str(), BindingFromJson(bj)); }
            }
        }
        if (root.contains("axes")) {
            for (const auto& [name, obj] : root.at("axes").items()) {
                if (obj.contains("positive")) {
                    for (const auto& bj : obj.at("positive")) { loaded.map.BindAxisPositive(name.c_str(), BindingFromJson(bj)); }
                }
                if (obj.contains("negative")) {
                    for (const auto& bj : obj.at("negative")) { loaded.map.BindAxisNegative(name.c_str(), BindingFromJson(bj)); }
                }
            }
        }
    } catch (...) {
